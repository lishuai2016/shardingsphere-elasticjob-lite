...
public final class FailoverListenerManager extends AbstractListenerManager {
    ...
    class JobCrashedJobListener extends AbstractJobListener {
        ...
        protected void dataChanged(final String path, final Type eventType, final String data) {
            if (isFailoverEnabled() && Type.NODE_REMOVED == eventType && instanceNode.isInstancePath(path)) {//jobName/instances节点变化
                String jobInstanceId = path.substring(instanceNode.getInstanceFullPath().length() + 1);//变化的节点
                if (jobInstanceId.equals(JobRegistry.getInstance().getJobInstance(jobName).getJobInstanceId())) {//判断是自己的节点变化返回
                    return;
                }
                List<Integer> failoverItems = failoverService.getFailoverItems(jobInstanceId);//获取分配这个节点的分片列表
