 ...
 * @author zhangliang
 */
public final class TriggerListenerManager extends AbstractListenerManager {//监听注册中心节点变化触发作业调度
    ...
    class JobTriggerStatusJobListener extends AbstractJobListener {
        ...
        protected void dataChanged(final String path, final Type eventType, final String data) {
            ...
            instanceService.clearTriggerFlag();//清除触发节点标记
            if (!JobRegistry.getInstance().isShutdown(jobName) && !JobRegistry.getInstance().isJobRunning(jobName)) {
                // TODO 目前是作业运行时不能触发, 未来改为堆积式触发
                JobRegistry.getInstance().getJobScheduleController(jobName).triggerJob();//触发任务运行一次
            }
        }
    }
}
